...
<body>
    ...
    <div class="container">
        ...
        <div class="isi">
            <h3 style="text-align:center;">LAPORAN PEMBAYARAN BANK</h3>
            <h4 style="text-align:center;">{{Carbon\carbon::parse($start)->translatedFormat('d-F-Y')}} - {{Carbon\carbon::parse($end)->translatedFormat('d-F-Y')}}</h4>
            <table id="myTable" class="table table-bordered table-striped dataTable no-footer text-center" role="grid"
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $d->pendaftaran->user->name }}</td>
                        <td>{{ $d->validasi }}</td>
                        <td>{{Carbon\carbon::parse($d->tanggal_bayar)->translatedFormat('d-F-Y')}}</td>
                        <td>Rp. {{number_format($d->jumlah, 0, ',', '.')}},-</td>
                    </tr>
                    @endforeach
                    {{-- <tr>
                        <td colspan="5">Total Jemaah Pembatalan Keberangkatan</td>
                        <td colspan="">{{$total}}</td>
                    </tr> --}}
                </tbody>
            </table>
